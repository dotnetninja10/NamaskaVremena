...
namespace NamaskaVremena
{
    ...
    */
    ...
    public class PrayTime
    {
        ...
        {
            "Fajr", "Sunrise", "Dhuhr", "Asr", "Sunset", "Maghrib",
        ...
        public PrayTime()
        {
            ...
            this.methodParams[Custom] = new double[] {19, 0, 0, 0, 17};
        }
    ...
    public class DrugoVreme
    {
        ...
        private Values sun(byte m, double d, double h, double lg)
        {
            if (m < 7)
                d = 31*(m - 1) + d + h/24;
            else
        ...
        private double Round(double x, byte a)
        {
            double tmp = x%a;
            if (tmp < 0)
                tmp += a;
...
namespace PrayTime2
    ...
    public class PrayTime
    {
        ...
        public static int Moonsighting = 10;   // Department of Islamic Development, Malaysia
        ...
        public PrayTime()
        {
            ...
            this.methodParams[Moonsighting] = new double[] { 18, 0, 0, 0, 18, 0 };
        }
        ...
        public double[] sunPosition(double jd)
        {
            ...
            Console.WriteLine("Angle {0}",d);

            //var tr =  (-this.dsin(0) - (this.dsin(L)*this.dsin(D)))/(this.dcos(L)*this.dcos(D));
            //var re = this.darccos(tr)/15;
            return new double[] { d, EqT };
        }
        ...
        public double[] computeTimes(double[] times)
        {
            ...
            if (calcMethod == Moonsighting)
            {

                Fajr = this.GetFajrMoonSight(Sunrise);
        ...
        private double GetFajrMoonSight(double sunRise)
        {
            //sunRise+=this.timeZone - this.lng / 15;
            //var tr=floatToTime24(sunRise);
            var minutes= CalculateFarjMinimumGeneral(this.lat, JulianToDateTime(this.JDate));
        ...
        public double[] adjustHighLatTimes(double[] times)
        {
            ...
            Console.WriteLine(24.00-nightTime);
            // Adjust Imsak
        ...
        public double[] adjustTimes(double[] times)
        {
            for (int i = 0; i < times.Length; i++)
            {
                if(calcMethod==Moonsighting && i!=1)
                times[i] += this.timeZone - this.lng / 15;
            }
