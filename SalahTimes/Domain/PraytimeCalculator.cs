...
namespace SalahTimes.Domain
{
    ...
    public class PraytimeCalculator
    {
        ...
        public static int Morocco = 7;    // Moroccan Habous
        public static int Malaysia = 8;   // Department of Islamic Development, Malaysia
        public static int Custom = 9;    // Custom Setting
        public static int Moonsight = 10;   // Department of Islamic Development, Malaysia
        ...
        private int calcMethod = Moonsight;		// caculation method
        private int asrJuristic = Shafii;	// Juristic method for Asr
        private double dhuhrMinutes = 0.0;		// minutes after mid-day for Dhuhr
        private double maghribMinutes = 0.0;
        private double ishaMinutesAfterMaghrib = 0.0;
        private int imsakUseMinutes = 1;      //minutes before Fajr for Imsak
        private int maghribUseMinutes = 1;
        private double imsakValue = 10.0;
        ...
        private int numIterations = 5;		// number of iterations needed to compute times
        ...
        {
            this.setCustomParams(new double[] { angle, -1, -1, -1, -1, -1 });
        }

        //// set the angle for calculating Maghrib
        public void setMaghribAngle(double angle)
        {
            this.setCustomParams(new double[] { -1, 0, angle, -1, -1, -1 });
        }

        // set the angle for calculating Isha
        public void setIshaAngle(double angle)
        {
            this.setCustomParams(new double[] { -1, -1, -1, 0, angle, -1 });
        }
        ///set custom values for calculation parameters
        public void setCustomParams(double[] param)
        {
            for (int i = 0; i < 6; i++)
            {
                if (param[i] == -1)
            ...
            //Ovdje treba da stoji =10 ako uzimas moonsight
        ...
        public void setDhuhrMinutes(double minutes)
        {
        ...
        {
            this.maghribUseMinutes = 1;
            this.maghribMinutes = minutes;
        ...
        {
            this.ishaMinutesAfterMaghrib = minutes;
            //this.setCustomParams(new double[] { -1, -1, -1, 1, minutes, -1 });
        }
        ...
        public String floatToTime24(double time)
        {
            if (time < 0)
                return InvalidTime;
            time = this.FixHour(time + 0.5 / 60.0);  // add 0.5 minutes to round
        ...
        public String[] getDatePrayerTimes(int year, int month, int day, double latitude,
                                            double longitude, double timeZoneC, int altitude)
        {
            this.DateTimeFormat = new DateTime(year, month, day);
            this.lat = latitude;
            this.lng = longitude;
            this.elv = altitude;
            this.timeZone = timeZoneC;
            this.JDate = this.JulianDate(year, month, day) - longitude / (15 * 24.0);
            var tr = JulianToDateTime(this.JDate);
            //var mojJDate = calcJulianDate(DateTimeFormat);
            //this.JDate = mojJDate;
        ...
        {
            double Z = this.FixHour(12.0 - DT[1]);
            return Z;
        }
        ...
        private double computeTime(double G, double[] DT)
        {
            ...
            double V = ((double)1 / 15.0) * this.darccos((-this.dsin(G) - this.dsin(DT[0]) * this.dsin(this.lat)) /
        ...
        public double[] computeTimes(double[] times)
        {
            ...
            double Fajr, Isha;
            if (this.calcMethod == Moonsight)
            {
                Fajr = MoonSightFarj(Sunrise);
                Isha = MoonSightIsha(Sunset);
            }
            else
            {
                Fajr = this.computeTime(180 - this.methodParams[this.calcMethod][0], t[1]);
                Isha = this.methodParams[this.calcMethod][3] == 1 ? 0 :
                            this.computeTime(this.methodParams[this.calcMethod][4], t[7]);
                //Isha = this.computeTime(this.methodParams[this.calcMethod][4], t[7]);
            ...
            //double Maghrib=this.computeTime(this.methodParams[this.calcMethod][2], t[6]);
        ...
        public double[] adjustHighLatTimes(double[] times)
        {
            ...
            // Adjust Maghrib
            double MaghribAngle = (methodParams[this.calcMethod][1] == 0) ? this.methodParams[this.calcMethod][2] : 4;
        ...
        //    }
        //    times[3] += this.dhuhrMinutes / 60.0; //Dhuhr
        //    if (this.imsakUseMinutes == 1) // Imsak
        //        times[0] = times[1] - imsakValue / 60.0;
        //    if (this.methodParams[this.calcMethod][1] == 1) // Maghrib
        //        //times[6] = times[5] + this.methodParams[this.calcMethod][2] / 60.0;
        //        times[6] = times[5] + this.maghribMinutes / 60.0;
        //    if (this.methodParams[this.calcMethod][3] == 1) // Isha
        //        times[7] = times[6] + this.methodParams[this.calcMethod][4] / 60.0;
        //        //times[7] = times[6] + this.ishaMinutes / 60.0;
        ...
        public double[] adjustTimes(double[] times)
        {
            ...
            if (this.imsakUseMinutes == 1) // Imsak
                times[0] = times[1] - imsakValue / 60.0;
            if (this.maghribUseMinutes > 0) // Maghrib
                times[6] = times[5] + this.maghribMinutes / 60.0;
            else
                times[6] = times[5] + this.methodParams[this.calcMethod][3]/60.0;
            if (this.ishaMinutesAfterMaghrib > 0) // Isha
                //times[7] = times[6] + this.methodParams[this.calcMethod][5] / 60.0;
                times[7] = times[6] + this.ishaMinutesAfterMaghrib / 60.0;
...
//        private int calcMethod = MWL;		// caculation method
//        private int asrJuristic = Shafii;	// Juristic method for Asr
//        private double dhuhrMinutes = 0;		// minutes after mid-day for Dhuhr
//        private double imsakUseMinutes = 1;      //minutes before Fajr for Imsak
...
//        public void setFajrAngle(double angle)
//        {
//            this.setCustomParams(new double[] { (int)angle, -1, -1, -1, -1, -1 });
//        }

//        // set the angle for calculating Maghrib
//        public void setMaghribAngle(double angle)
//        {
//            this.setCustomParams(new double[] { -1, 0, (int)angle, -1, -1, -1 });
//        }

//        // set the angle for calculating Isha
//        public void setIshaAngle(double angle)
//        {
//            this.setCustomParams(new double[] { -1, -1, -1, 0, (int)angle, -1 });
//        }
...
//        {
//            this.setCustomParams(new double[] { -1, 1, minutes, -1, - 1, -1 });
//        }
...
//                times = this.computeTimes(times);
//            }
//            var timesBeforeAdjusting1 = times;
//            var adjtimes1=this.adjustTimesFormat(timesBeforeAdjusting1);
...
//            var timesAfterAdjusting2 = times;
//            var adjtimes2 = this.adjustTimesFormat(timesAfterAdjusting2);
